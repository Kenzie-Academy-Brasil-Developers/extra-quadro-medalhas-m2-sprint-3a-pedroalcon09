...
async function getCountry() {
    const arrayCountry = await fetch('https://kenzie-olympics.herokuapp.com/paises')
...
function buildTable(array) {
    ...
    for (let i = array.length - 1; i >= 0; i--) {
        ...
        total.innerText = array[i].medal_gold + array[i].medal_silver + array[i].medal_bronze
...
function sortGold() {
    const sorted = countryArray.sort(function(a, b)  {
        if (a.medal_gold > b.medal_gold) {return 1;}
        if (a.medal_gold < b.medal_gold) {return -1;}
        if (a.medal_gold === b.medal_gold) {return 0;}
    })
    buildTable(sorted);
    return sorted;
}
...
function sortSilver() {
    const sorted = countryArray.sort(function(a, b) {
        if (a.medal_silver > b.medal_silver) {return 1;}
        if (a.medal_silver < b.medal_silver) {return -1;}
        if (a.medal_silver === b.medal_silver) {return 0;}
    })
    buildTable(sorted);
    return sorted;
}
...
function sortBronze() {
    const sorted = countryArray.sort(function(a, b) {
        if (a.medal_bronze > b.medal_bronze) {return 1;}
        if (a.medal_bronze < b.medal_bronze) {return -1;}
        if (a.medal_bronze === b.medal_bronze) {return 0;}
    })
    buildTable(sorted);
    return sorted;
}
...
function searchOneCountry() {
    let oneCountry = [];
    oneCountry.push(countryArray.find(elem => {
